...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profile?.is_admin) {
      ...
      if (remaining <= 0) {
        await addLog(campaignId, userId, 'quota_exceeded', 'Seu pacote de mensagens acabou. Entre em contato com suporte.');
        await supabaseClient.from('profiles').update({ account_status: 'paused' }).eq('id', userId);
        return new Response(JSON.stringify({ error: 'Seu pacote de mensagens acabou. Entre em contato com suporte.' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 403,
        });
      }
